...
Your internal state for every session is strictly the four structures defined in `tui-layer/state/term_series.py`:
- ActiveTermSeries
...
All tool use, state changes, and visible output must pass through the universal guard in `tui-layer/enforcement/surface_enclosure.py`.
